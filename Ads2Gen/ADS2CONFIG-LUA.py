...
LRU_TO_BOARD = {'FSIB_AFDX_01' :   ['HF_FCM_1','HF_FCM_2','HF_FCM_3'], #'NTM_L1', 'NTM_R3','NTM_L4', 
                'FSIB_AFDX_02' :   ['RGW04_Analog_In', 'RGW02_Analog_In', 'RGW02_A429_In', 'RGW02_A825_In','RGW04_A825_In', 'RGW06_A429_In','RGW03_Analog_In', 'RGW03_A429_In', 'RGW06_Analog_In','RGW06_A825_In', 'RGW05_Analog_In','RGW01_A825_In', 'RGW01_Analog_In', 'RGW05_A429_In'  ],
                'FSIB_AFDX_03' :   ['RGW07_Analog_In', 'RGW07_A429_In','RGW08_Analog_In', 'RGW08_A429_In',  'HF_AHMUINSTANCE'],
                'FSIB_AFDX_04' :   ['RGW09_A429_In', 'RGW10_A429_In','RGW11_Analog_In', 'RGW14_A429_In','RGW16_A429_In', 'RGW09_Analog_In', 'RGW15_Analog_In',  'RGW12_A429_In','RGW16_Analog_In', 'RGW10_Analog_In',  'RGW12_Analog_In', 'RGW15_A429_In'],
...
# all the signal in one iom is impossible, for that afdx board may break.    
def GenerateconfigLUA(mergedict,mode,devmap,split): # generate all the signal in one iom and cvt 
    templatedir = os.path.dirname(__file__)           
    #gac.cvts.clear()    
    ...
    for msg in mergedict.inputafdxmsg.values():
        if msg['SourceUDP'] not in udpports:
            udpports.append(msg['SourceUDP'])
    for msg in mergedict.outputafdxmsg.values():
        if msg["SourceUDP"] not in outputudpports:
            outputudpports.append(msg["SourceUDP"])
      
    print "len of UDP port list is %d " % len(udpports)
    
    for msg in mergedict.outputafdxmsg.values():
        sigdict = msg['Signals']
        sigtemp = {}
        for sig in sigdict.values():
            key = int(sig['sigbyteoffset']) * 8 + int(sig['sigbitoffset'])
            sigtemp[key] = sig
        msg["Signalss"] = sigtemp
    ...
    for udpport in udpports:
        ...
        tmpl = Cheetah.Template.Template(file= os.path.join(templatedir, "ads2genLUA.tpl"), 
                searchList=[{                    
                    "inputmodel" : inputmap,
                    "udpport": udpport,
                    }])
        ...
        fn = os.path.join(outdir, str(udpport) + '.lua')
        temfile = tmpl.respond()
        open(fn, "w").write(temfile)

    for udpport in outputudpports:
        #print lruname
                
        tmpl = Cheetah.Template.Template(file= os.path.join(templatedir, "ads2genLUA.tpl"), 
                searchList=[{                    
                    "inputmodel" : outputmap,
                    "udpport": udpport,
                    }])
...
class CVTConnect(object):
    def __init__(self,filename):
        ...
        self.cvts = dict()
        self.aliasname = dict()
    def newpoint(self,name,datatype,datasize,defaultvalue,elements=1,alias=[], varlength=0):          
        return Bunch(name = name,datatype=datatype,datasize = datasize, defaultvalue = defaultvalue,elements=elements,alias=alias, varlength =varlength)
    ...
    def add_664_signal(self,msg,sig):
        ...
        if sig['direct'] == 'Input':            
            ...
            name = message['fullname']+'.'+sig['DataSet']+'.'+sig['DpName']
            alias.append(name)
        #else:
    ...
    def add_664_fsb(self,msg,sig):
        ...
        if sig['direct'] == 'Input':
            ...
            alias.append(message['fullname']+'.'+sig['DataSet']+'.'+sig['DpName']+'.__fsb__')
...
def ProcessExcel(workdir,outdir, hfnamelist,platform,merge,mode,devmapfile,split):
    ...
    if merge == True:
        ...
        GenerateconfigLUA(AnalysisRecord,mode,devmap,split)
