...
#def IsRawMsg(m):
    ##$print("message protocal is %s " % ($m['MsgDataProtocolType']))
    #if '661' in $m['MsgDataProtocolType'].lower()
        #return True
    #else
        #return False
    #end if
...
#def gen_signal(m, p)
		...
		#end if
	#end if 
	##$print("message protocal is %s " % ($p['Pubref']))
	#if p.get('RpName') is not None
		#if 'spare' in $str($p['RpName']).lower().split(".") or 'reserve' in  $str($p['RpName']).lower().split("."):
            ##$print("######################################### %s " % ($str($p['RpName']).lower()))
			#return		
		#end if
	#end if
...
#for $messageName in $model.keys()   
    #set $message=$model[$messageName]
	    ...
	    #if $IsRawMsg($message) == True                   
                            MSG_CONTENT rawdata = {
	                            MESSAGE_TYPE AFDX = { }
                                cvtslot = $cvts.add_664_signal($message,$message['Signals'].values()[0])
	                        }
	    #else
                            MSG_CONTENT signals = {
	                            MESSAGE_TYPE AFDX = { }
...
##     #if 'crcapplication' in $message and $message['crcapplication'] == True
##                #set $sbyte = $message['crcsignal']['sigbyteoffset']
##                #set $ebyte = $sbyte + 3
##                            ALIC enable_alic = {
##                                alic_fs_offset = "$message['crcsignal']['FsbOffset']";
##                                alic_pos = "$sbyte:$ebyte";
##                                alic_crc_type = "ARINC-665";
##                                Mode ei_mask = {
##                                    alic_crc_poly = "0x04c11db7";
##                                    alic_crc_init = "0xffffffff";
##                                    alic_crc_final = "0xffffffff";
##                                    alic_fs_value = "$cvts.add_664_control($message, '__CRC_FSB__')";
##                                    alic_value = "$cvts.add_664_control($message, '__CRC_VAL__')";
##                                }
##
##                            }
##     #else
...
##     #end if
	                    ...
	                    iomap {    
     #if $IsRawMsg($message) == False
	                            #for $sig in $message['Signals'].values()
                                     ##$print("message name is %s " % ($sig))
...
##                                    alic_crc_init = "0xffffffff";
##                                    alic_crc_final = "0xffffffff";
##                                    alic_fs_value = "$cvts.add_664_fsb($message,$message['crcsignal'])";
##                                    alic_value = "$cvts.add_664_signal($message,$message['crcsignal'])";
##                                }
##
##                            }
##     #else
