#def gen_signal(msg, p)
	...
	#set $endSigByte = ( $p['sigbitoffset'] + $p['SigSize']-1)/8
	#set $startSigByte 	= ( $p['sigbitoffset'])/8
	#if ($endSigByte - $startSigByte+1)>2
		#set $endSigByte = $endSigByte / 4 * 4 + 3
		#set $startSigByte 	= $startSigByte /4 * 4
		#set $startSigBit 	= ( $p['sigbitoffset']) - $p['sigbitoffset']/32 * 32 
		#set $endSigBit 	= ( $p['sigbitoffset']) + $p['SigSize'] - $p['sigbitoffset']/32 * 32 -1
	#else
...
#def gen_inputmsg(model, dev)
	#for $messageName in $model.keys()
		...
		#if $LruName is None or ($message['Lru'] in $LruName and $message['PhysPort'] in $dev['Lru_Channel'])
	        $print("come 2")
