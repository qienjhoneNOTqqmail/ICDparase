...
def Count_429_For_RDIU(signals):
    lrucount={}
    for sig in signals.values():
        #print type(sig)
        #print sig['Lru']
        if sig['Lru'].startswith('RGW'):
            
            origianllru = sig['Pubref'].split('.')[0]
            port = sig['Pubref'].split('.')[1]
            val = port+'.'+sig['DataSet']
            if origianllru not in lrucount:
                lrucount[origianllru] = [val]
            else:
                if val not in lrucount[origianllru]:
                    lrucount[origianllru].append(val)
                    
    for key, item in lrucount.items():
        print "****************************"
        print key,
        print ':',
        print len(item),
...
def ProcessExcel(workdir,outdir, hfnamelist,platform,merge,mode,devmapfile,split):
    ...
    if merge == True:
        ...
        afdxsignals = AnalysisRecord.inputafdxsignal
        Count_429_For_RDIU(afdxsignals)
