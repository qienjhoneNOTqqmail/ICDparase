...
##for UDP monitor generate signal
#def gen_signalUDP(m, p)
	#if p.get('DpName') is not None
		#if 'FSB' in p['DpName'] or 'spare' in $str($p['DpName']).lower() or 'reserve' in  $str($p['DpName']).lower():
			#return
		#end if
	...
	#end if
    #if $p['SigSize'] > ($p['sigalign'] * 8)
       #set $endSigByte = $p['sigbyteoffset']+$p['SigSize'] / 8 - 1 + 78
    #else
       #set $endSigByte = $p['sigbyteoffset']+$p['sigalign'] - 1 + 78
    #end if
    #set $startSigByte = $p['sigbyteoffset'] + 78
    #set $endSigBit  = $p['sigbitoffset']+$p['SigSize']-1
                        Sample {
                            point = "$cvts.add_664_signal($m,$p)";
    ...
    #if $p['664FSB']
       #set $fsbname = $cvts.add_664_fsb($m,$p)
       #set $fsbstart = $p['FsbOffset'] + 78
       #if $fsbname 
                        Sample {
                            point = "$fsbname";
                            bytes = "$fsbstart:$fsbstart";
                            bits = "0:7";
                            ppexpr = "";
                            COPY euconvert = {
                            }
...
## ----------------------------------------------------------------------------
## receive message for stimulation mode by TCP-UDP, not AFDX
## Constructed from a output ICD of the LRU
## ----------------------------------------------------------------------------

#def gen_rxStimMsgUDP($model)
#for $messageName in $model.keys()
    #if "A661" in $messageName or "BLOCK" in $messageName
        #continue
    #end if
    #set $message=$model[$messageName]
    #if $LruName is None or $message['Lru'] in $LruName 
                   ...
                   Message $message['Message'] = {
                        FILTER {
                            ...
                            IDENTIFIER msgident = {
                                ...
                                mask = "0xffffffff";
                                match = "$cvts.add_monitor_signal($message)";
                            }
...
IOMAP {
    ...
    #if $LruName in ['SYNOPTICEMENUAPP_L','SYNOPTICMENUAPP_R','SYNOPTICPAGEAPP_L','SYNOPTICPAGEAPP_R','VIRTUALCONTROLAPP_L','VIRTUALCONTROLAPP_R','FDAS_L1','FDAS_L3','FDAS_R3','HF_IDULEFTINBOARD','HF_IDULEFTOUTBOARD','HF_IDUCENTER','HF_IDURIGHTOUTBOARD','HF_IDURIGHTINBOARD','IMA_DM_L4','IMA_DM_L5','IMA_DM_R4']:
    #set $devlist = $device.split(",")
    TRANSPUTS {
        TCP-UDP $devlist[0] = {
            description {
                comment = "";
