...
from openpyxl.styles import Color, Font, Side, Border, PatternFill, NamedStyle
...
normstyle = NamedStyle(name='normstyle', border=border)


def genExcelSheet(wb, title, descriptor, data):
    ws = wb.create_sheet(title=title)
    try:
        wb.add_named_style(hdrstyle)
...
if __name__ == "__main__":
    x = readExcelFile("HF_IDUCENTER-icd.xlsx",
                      ('InputMessages', 'InputSignals'))
    print len(x)
    for lst in x:
        print len(lst)
