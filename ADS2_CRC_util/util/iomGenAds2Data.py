...
class CvtCollector():
    ...
    def add_a664_msg_control(self, message, function):
        ...
        attribs = self.newpoint(name=name, datatype="uint32", datasize=4, aliases=[name], defaultvalue=defaultvalue)
        self.cvtpoints[name] = attribs
        return self.cvtname + '::' + name
    ...
    def add_a664_a429ssm(self, param): #assuming NON-SSM param
        ...
        if isinstance(param, iomGenCommon.InputSignal):
            ...
            aliases.append(".".join([message.fullname,param.dsName, a429wordname, "SSM" ]))
        if param.paramName is not None:
    ...
    def add_a429_signal(self, param):
            ...
            if isinstance(param, iomGenCommon.InputSignal):
                ...
                alias = message.fullname + '.' + param.sigName
            else:
                return None
            aliases.append(alias)
            ...
            #add alias linked to corresponding SSM
            ssmName = message.fullname + '.SSM'
            if not self.cvtpoints.has_key(ssmName):
                 attribs = self.newpoint(name=ssmName, datatype='int32', datasize=4, aliases=[ssmName], defaultvalue=1)
            else:
                 attribs = self.cvtpoints[ssmName]
            attribs.aliases.append(alias+".__ssm__")
            self.cvtpoints[ssmName] = attribs
    ...
    def add_a429_msg_control(self, label, function):
        ...
        if function == 'ssm': #FIXME
            defaultvalue = "1"
            name = label.fullname + ".SSM" # __ssm__ added as aliases to signal names
        else:
            defaultvalue = "0"
...
def main(args):
    ...
    target     = args[5]        #
    devmode	   = False          # for targets SSDL, SDIB False otherwise True
    if len(args) > 6:
    ...
    if target == "SWTESTS" or target == "MDVS":
        devmode = True
            
    if target == "SDIB" or target == "SSDL":
        devmode = False
