...
def genSwCfg(data, vlan_lst, outfn):
	...
	for app in data:
		...
		for vl in app[1].afdx.vls.values(): #input vl
			...
			print "====check lru name is %s"% (vl.lru)
			if getIdxPort(vl.lru, vlan_lst) is None:
				print ("can't find %lru in devmap"% vl.lru)
				continue
			port =  vlan_lst[getIdxPort(vl.lru, vlan_lst)][0]
			if not port:
				print ("can't find %lru in devmap"% vl.lru)
				continue
	...
	for mapping in devMap.iter_rows(row_offset=1):
		...
		if(lru not in lrus):
			try:
				...
				print("Lru:%s, UUT_port:%d, SIM_Port:%d\n"%(lru,port_uut,port_mon))
