...
# RCS-ID:       $Id: Main.py,v 1.168.2.11 2007/03/20 17:36:00 RD Exp $
...
# only shows the extension(s) you want it to.
    ...
    wildcard = "Initial Config (*.ini)|*.ini|"     \
               "Python source (*.py)|*.py|"     \
    ...
    def __init__(
        ...
        label = wx.StaticText(self, -1, "Configuration file")
        label.SetHelpText("Choose the configure file for tool")
        sizer.Add(label, 0, wx.ALIGN_CENTRE|wx.ALL, 5)

        box = wx.BoxSizer(wx.HORIZONTAL)

        label = wx.StaticText(self, -1, "Path:")
        label.SetHelpText("This is the help text for the path")
        box.Add(label, 0, wx.ALIGN_LEFT, 5)

        self.text = wx.TextCtrl(self)#, -1, "", size=(80,-1))
        self.text.SetHelpText("Here's some help text for path")
        box.Add(self.text, 2)
        
        btnopenfile = wx.Button(self, -1, "Scan")
        self.Bind(wx.EVT_BUTTON, self.OnButton, btnopenfile)
        box.Add(btnopenfile, 1, wx.ALIGN_RIGHT, 5)     
    ...
    def OnButton(self, evt):
        ...
        dlg = wx.FileDialog(
            ...
            defaultFile="", wildcard=self.wildcard, style=wx.OPEN | wx.MULTIPLE | wx.CHANGE_DIR
            )
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.text.SetValue(paths[0])
            
            self.par.readIniFile(paths[0])
...
class wxPythonDemo(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        self.readIniFile(r'D:\C919Tools\C919_ICD_Processor\ICD_Process.ini')
        ...
        #icon = images.getWXPdemoIcon()  # change to savic icon
        #self.SetIcon(icon)

        try:                       # delete this than can't close the app, for that it call the self.tbicon in OnCloseWindow
        ...
        for item in _treeList:
            ...
            if hasitem:
                ...
                mi = menu.Append(-1,item[0],'first child')
                self.Bind(wx.EVT_MENU, self.OnDemoMenu, mi)
        self.mainmenu.Append(menu, '&Func')
       ...
       # 
        ...
        menu.AppendSeparator()
        helpItem = menu.Append(-1, '&About ICD Processor Tool', 'Tool Introduce!!!')
        ...
        root = self.tree.AddRoot("Tool Overview")
        firstChild = None
        for item in _treeList:
    ...
    def OnTreeLeftDown(self, event):
        ...
        if item == self.tree.GetSelection():
            pass#self.SetOverview(self.tree.GetItemText(item)+" Overview", self.curOverview)
        event.Skip()
    ...
    def UpdateNotebook(self, select = -1):
        ...
        UpdatePage(self.demoPage, self.demoModules.name)
        
        if self.demoModules.name not in self.NBPage:
            self.NBPage[self.demoModules.name]= nb.GetPageCount()
        
        #if self.demoModules.name == 

        if select >= 0 and select < nb.GetPageCount():
            nb.SetSelection(self.NBPage[self.demoModules.name]-1)
    ...
    def readIniFile(self, path):
        self.iniFile = path #os.path.join('D:\C919Tools\C919_ICD_Processor', 'ICD_Process.ini')
        self.cfp = ConfigParser.ConfigParser()
        self.section = 'ICDImport'
        self.sectionads2 = 'Ads2Gen'
        if os.path.isfile(self.iniFile):
            self.cfp.read(self.iniFile)
            try:
                self.genConfigData['hflist'] = self.cfp.get(self.section, 'hflist')
                self.genConfigData['outputdir'] = self.cfp.get(self.section, 'outputdir')
                self.genConfigData['loglevel'] = self.cfp.get(self.section, 'loglevel')
                self.genConfigData['xmlICDpath'] = self.cfp.get(self.section, 'xmlICDpath')
                self.genConfigData['devicemap'] = self.cfp.get(self.sectionads2, 'devicemap')
                self.genConfigData['ads2outdir'] = self.cfp.get(self.sectionads2, 'ads2outdir')
                self.genConfigData['target'] = self.cfp.get(self.sectionads2, 'target')
                self.genConfigData['simmode'] = self.cfp.get(self.sectionads2, 'simmode')
                self.genConfigData['msgmode'] = self.cfp.get(self.sectionads2, 'msgmode')
                self.genConfigData['excelpath'] = self.cfp.get(self.sectionads2, 'excelpath')
                self.genConfigData['hfnames'] = self.cfp.get(self.sectionads2, 'hfnames')
            except:
                self.log.WriteText("unknown INI File %s" % path)
                
    def readStdErr(self, p):
        try:
    ...
    def waitStdErr(self):
        try:
            ...
            s = self.StdErrQueue.get_nowait()
            if s:
                self.log.WriteText("INFO: %s\n" % s)
        except Queue.Empty:
            pass
            
        wx.FutureCall(10, self.waitStdErr)
        

    def OnToggleRedirect(self, event):
        dlg = SetupDialog(self, -1, "Setup Dialog", size=(100, 100),
...
class MySplashScreen(wx.SplashScreen):
    ...
    def ShowMain(self):
        frame = wxPythonDemo(None, "SAVIC: C919 ICD Processor V&V ")
        frame.Show()
        if self.fc.IsRunning():
            self.Raise()
