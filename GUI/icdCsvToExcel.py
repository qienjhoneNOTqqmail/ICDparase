...
def convertToDictCsv(filename):
    # Read CSV Export from DOORS 
    global paramInputColumns
    global csvSeparator
    global csvLfChar
    ...
    title = inputlines[0].split(csvSeparator)[:-1]
...
def convertToDict(filename):
    if os.path.exists(filename):
        if filename.endswith(".csv"):
            return convertToDictCsv(filename)
        elif filename.endswith('.xlsx'):
            return convertToDictXls(filename)
...
def usage():
    sys.stderr.write('Usage: iomGenIcd outputfile afdxMsgIn canMsgIn a429MsgIn paramIn afdxMsgOut canMsgOut a429MsgOut paramOut\n\n')
