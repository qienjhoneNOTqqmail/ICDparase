...
#def isStatus(p)
    #try
        ...
        #if "FSB" in $p.sigName or "SSM" in $p.sigName
            #return 1
        #else
            ## inhibit also complete 429 words - are already considered in ICD as signals
            #if $p.sigName.startswith("L") and $p.sigType=="BYTES" and $p.granularity=="" and $p.sigSize==32
                #return 1
            #else
                #return 0
            #end if
    ...
    #try
        ...
        #if "FSB" in $p.DpName or ".SSM" in $p.DpName 
            #return 1
        #else    
            #return 0
        #end if
      ...
      #set $porttype = $message.portType
		 ...
		 #set $porttype = "SAMPLING"
                        ...
                        PORT_TYPE $porttype.lower() = {
                            ...
                            #set $txrate = $message.msgTxInterval / 10 + ($message.msgTxInterval % 10 > 0)
                            #if $txrate == 0
                               #set $txrate = 1
                            #end if
                            tx_rate = "$txrate";
  ...
  #for $messageName in $model.afdx.messages.keys()
    ...
    #set $vl=$model.afdx.vls[$message.vlId]
       ...
       #set $queuesize = $vl.mtu*$message.queueLength
                        ...
                        #set $rate = $message.msgTxInterval / 10 + ($message.msgTxInterval % 10 > 0)
                        #if $rate == 0
                            #set $rate=1
                        #end if
                        EXPECTED_RATE = "$rate";
