...
class TestPanel(wx.Panel):
    def __init__(self, parent, log, frame):
        ...
        '''
        self.icdpathtext = wx.TextCtrl(self, -1, frame.genConfigData['excelpath'], pos=(100,10), size=(500,-1))
        ...
        self.outputtext = wx.TextCtrl(self, -1, frame.genConfigData['ads2outdir'], pos=(100,40), size=(500,-1))
        ...
        self.hflisttext = wx.TextCtrl(self, -1, frame.genConfigData['hfnames'], pos=(100,70), size=(500,100), style=wx.TE_MULTILINE)
        self.hflisttext.SetHelpText("Here's some help text for path")
        
        labeldevmap = wx.StaticText(self, -1, "Device Map", (10,100))
        labeldevmap.SetHelpText("Output diectory the ICD file path for tool")

        
        self.devmaptext = wx.TextCtrl(self, -1, frame.genConfigData['devicemap'], pos=(100,100), size=(500,100), style=wx.TE_MULTILINE)
        ...
        rb_mode = wx.RadioBox(
                self, -1, "Mode", pos=(10,130), wx.DefaultSize,
...
#iomGenExcel.py --merge --loglevel=ERROR --output=%MERGED_ICD% %FUN_LIST% -- "%ICD_PATH%"
    def OnButton(self, evt):
        self.log.WriteText(self.PYTHON + ' ' + 'iomGenExcel.py' + ' --merge --loglevel=ERROR --output=' + self.outputtext.GetValue()+ ' -- '+ self.icdpathtext.GetValue())
        try:
            self.compile = subprocess.Popen(
                               self.PYTHON + ' ' + 'D:\C919Tools\C919_ICD_Processor\GUI\iomGenExcel.py' + ' -c -m --loglevel=INFO --inputtype=Excel --outdir=' + self.outputtext.GetValue()+ ' --mode=' + self.smode+ ' --devmap='+ self.devmaptext.GetValue()+ ' ' +self.hflisttext.GetValue()+ ' -- '+ self.icdpathtext.GetValue(),
                               shell=False,
